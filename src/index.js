...
const BRAIN_PROCESS_TIMEOUT_MS = Number(env('BRAIN_PROCESS_TIMEOUT_MS', '120000'));
const BRAIN_PROCESS_URL = env('BRAIN_PROCESS_URL', 'https://flybot-brain.onrender.com/brain/process');
const BRAIN_RETRIES = Math.max(1, Number(env('BRAIN_RETRIES', '6')));
const BRAIN_WARMUP_MS = Math.max(60000, Number(env('BRAIN_WARMUP_MS', '600000')));
const BRAIN_HEALTH_TIMEOUT_MS = Math.max(5000, Number(env('BRAIN_HEALTH_TIMEOUT_MS', '15000')));
...
function cleanError(raw, status) {
  ...
  if (status === 502 || text.includes('502')) return 'FlyBrain is waking up on Render (502 gateway).';
  if (status === 503 || text.includes('503')) return 'FlyBrain is starting on Render (503 unavailable).';
  if (status === 504 || text.includes('504')) return 'FlyBrain processing timed out.';
  if (status === 501 || text.includes('501')) return 'FlyBrain endpoint is not POST-capable on the current deployment.';
  return text.slice(0, 800) || `FlyBrain HTTP ${status}`;
}

async function brainHealthCheck() {
  const { controller, timer } = timeoutSignal(BRAIN_HEALTH_TIMEOUT_MS);
  try {
    const response = await fetch(BRAIN_PROCESS_URL.replace('/brain/process', '/health'), {
      method: 'GET',
      signal: controller.signal,
      headers: { 'User-Agent': 'FlyBot-Warmup/1.0' }
    });
    const raw = await response.text();
    if (!response.ok) throw new Error(cleanError(raw, response.status));
    let payload = {};
...
async function warmFlyBrain(reason = 'periodic') {
  try {
    const health = await brainHealthCheck();
    console.log(`[FlyBrain] warmup OK (${reason}) ${JSON.stringify(health).slice(0, 500)}`);
  } catch (error) {
    lastBrainError = error.message;
    console.warn(`[FlyBrain] warmup ${reason} failed: ${error.message}`);
  }
}
...
async function processThroughFlyBrain(text, author) {
  ...
  for (let attempt = 1; attempt <= BRAIN_RETRIES; attempt += 1) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(cleanError(payload.error || raw, response.status));
      return payload;
    } catch (error) {
      lastError = error;
      lastBrainError = error.message;
      console.warn(`[FlyBrain] attempt ${attempt}/${BRAIN_RETRIES}: ${error.message}`);
      if (attempt < BRAIN_RETRIES) {
        await warmFlyBrain(`retry-${attempt}`);
        await new Promise(resolve => setTimeout(resolve, Math.min(15000, 2000 * attempt)));
      }
    } finally {
      clearTimeout(timer);
    }
...
async function sendFlyMessage(payload) {
  ...
  const now = Date.now();
  if (now - lastActionAt < COOLDOWN_MS) return { sent: false, reason: 'Action cooldown active.', action };
...
function formatBrainReply(input, result) {
  ...
  if (result?.stimulus) lines.splice(3, 0, `**Stimulus:** ${result.stimulus}`);
  return lines.join('\n');
}
...
async function replyToMessage(message) {
  ...
  console.log(`[Discord] received realtime message: ${message.author.tag || message.author.id} -> ${text.slice(0, 300)}`);

  try {
    const result = await processThroughFlyBrain(text, message.author);
    ...
    messagesSent += 1;
    console.log(`[FlyBrain] processed ${text.slice(0, 120)} -> ${JSON.stringify(result).slice(0, 1500)}`);
...
client.once('ready', async () => {
  ...
  await warmFlyBrain('startup');
  warmupTimer = setInterval(() => void warmFlyBrain('interval'), BRAIN_WARMUP_MS);
  warmupTimer.unref?.();
